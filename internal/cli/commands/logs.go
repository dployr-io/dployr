...
func fetchLogs(d *deps, serviceID, source string, since time.Time, limit int) error {
	...
	if err != nil {
		...
		if isNotImplemented(err) {
			return fmt.Errorf("centralized logs are not yet available on this server: requires an updated dployr-base with the /v1/services/:id/logs endpoint")
		}
		return err
	}
...
func streamLogs(d *deps, serviceID, source string) error {
	...
	if err != nil && isNotImplemented(err) {
		return fmt.Errorf("real-time log streaming is not yet available on this server: requires an updated dployr-base with the /v1/services/:id/logs?follow=true endpoint")
	}
	return err
}
