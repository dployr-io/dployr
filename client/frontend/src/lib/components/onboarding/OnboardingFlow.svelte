<script lang="ts">
  ...
  const pages = [
    ...
    },
    {
      ...
      options: ["New project idea", "Existing project"],
    },
    {
  ...
  async function handleMagicCode(host: string, email: string, code: string) {
    try {
      ...
      saveToLocalStorage("token", res.access_token);
  ...
  $: canProceed = (() => {
    ...
    if ($currentPage === 1)
      return (
        ...
        (!$discoveryOptions.includes("Other") ||
          $discoveryOther.trim().length > 0)
      );
...
<main class="w-full flex items-center justify-center min-h-screen">
  <div
    class="rounded-xl p-12 transition-all duration-300 ease-in-out {$isTransitioning
      ? 'opacity-70 translate-x-5'
  ...
  >
    <div class="flex flex-col items-center gap-6 mb-8">
      ...
      />
      <div
        class="text-xl font-semibold text-center flex-1 leading-relaxed text-gray-700 dark:text-gray-100"
      >
        {#if !$currentUser}
      ...
      {#if !$currentUser && $currentPage === 3}
        <div class="flex flex-col items-start">
          ...
          <p>
            <a href="" class="underline font-semibold">Click</a> to learn how.
          </p>
        </div>
      {/if}
    </div>
    ...
    <OnboardingPage
      ...
      {toggleDiscovery}
      {selectAppStage}
      handleSignIn={async () => handleSignIn($host, $email, $name)}
      handleMagicCode={async () => handleMagicCode($host, $email, $otp)}
      {nextPage}
      {previousPage}
      {canProceed}
